...
class Critic(nn.Module):
  ...
  def __init__(self , action_dim ):
    ...
    self.features4 = nn.Sequential(
        nn.Linear(64+action_dim,300), # state dim + Action dim
        nn.ReLU(),
        nn.Linear(300,400 ),
        nn.ReLU(),
        nn.Linear(400,1)
...
class TD3(object):
  ...
  def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5,
    ...
    for it in range(iterations):
      ...
      state = torch.Tensor(batch_states).unsqueeze(1)
      next_state = torch.Tensor(batch_next_states).unsqueeze(1)
